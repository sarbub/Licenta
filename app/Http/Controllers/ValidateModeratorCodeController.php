...
class ValidateModeratorCodeController extends Controller
{
    //

    public function ValidateNaratorCodeControllerMainMethod(request $request){
        $data = $request->all();
        ...
        if ($validator->fails()) {
            return back()
                ->withErrors($validator, 'narator_code_error_bag')
                ->withInput();
        }

        $session_code = (string) session('NaratorEmailValidationCode');
