...
<body>
    ...
    <div class="main_validation_code_div">
        <section class="col-6">
            <h1>Ne este narator? Devino tu unul</h1>
            <h2>Acest cont va fi facut narator automat in cazul lipsei acestuia</h2>
        </section>
        <section class="col-6">
            <form id="codeForm" method="POST" action="{{ route('ModeratorCodeValidation.form') }}">
                @csrf
                @if ($errors->narator_code_error_bag->any())
                ...
                @endif
                @if(session('NaratorCodeSuccess'))
                <div class="alert alert-success">
                    {{ session('NaratorCodeSuccess') }}
                </div>
                @endif
                <div class="d-flex gap-4">
                    <input type="text" maxlength="1" class="code-input @if($errors->narator_code_error_bag->has('digit1')) is-invalid @endif" name="digit1" inputmode="numeric" value="{{ old('digit1') }}" />
                    <input type="text" maxlength="1" class="code-input @if($errors->narator_code_error_bag->has('digit2')) is-invalid @endif" name="digit2" inputmode="numeric" value="{{ old('digit2') }}" />
                    <input type="text" maxlength="1" class="code-input @if($errors->narator_code_error_bag->has('digit3')) is-invalid @endif" name="digit3" inputmode="numeric" value="{{ old('digit3') }}" />
                    <input type="text" maxlength="1" class="code-input @if($errors->narator_code_error_bag->has('digit4')) is-invalid @endif" name="digit4" inputmode="numeric" value="{{ old('digit4') }}" />
                    <input type="text" maxlength="1" class="code-input @if($errors->narator_code_error_bag->has('digit5')) is-invalid @endif" name="digit5" inputmode="numeric" value="{{ old('digit5') }}" />
                    <input type="text" maxlength="1" class="code-input @if($errors->narator_code_error_bag->has('digit6')) is-invalid @endif" name="digit6" inputmode="numeric" value="{{ old('digit5') }}" />
                </div>
            ...
            <form method="POST" id="resend_code_form" class="mt-3" action="{{ route('ModeratorCodeValidation.form') }}">
                @csrf
                <input type="hidden" name="email" value="{{ session('naratorEmail') }}">
                <div class="d-flex">
                    <p>Nu ai primit cod?</p>
                <button type="submit" id="resend_code_btn" class="btn btn-link" {{ session('naratorEmail') ? '' : 'disabled' }}>
